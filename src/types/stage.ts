...
/** Top-level area (world/continent) identifier. */
export type AreaId = string;

/** Zone identifier within an area (e.g. "area_01_z03"). */
...
/** A single item that a mob can drop, with its drop probability. */
...
/** Bonus types available as stage modifiers. */
...
export interface Stage {
  ...
  bonuses: StageBonus[];
  /** Whether this stage has a boss encounter (every 10th stage in a zone). */
  hasBoss: boolean;
 ...
 * Omits mob details to keep large indexes fast to load.
 */
...
/** Stage index keyed by stage id for O(1) lookup. Built at runtime from STAGES. */
